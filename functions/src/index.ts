...
export const onAppointmentWritten = functions.firestore.document("/appointments/{appointmentId}")
    .onWrite(async (change, context) => {
            ...
            try {
                ...
                const notificationPromises = userIdsToNotify.map(userId => 
                    sendPushNotification(
                        ...
                        { route: '/admin/appointments/pending', notificationId: `pending-${appointmentId}` }
                    )
                );
        ...
        // 0. Appointment created directly by admin
        if (!dataBefore && dataAfter?.status === 'scheduled') {
            functions.logger.info(`Appointment ${appointmentId} created by admin. Enqueueing notifications.`);
            const title = 'נקבע לך תור חדש!';
            const body = `היי ${clientName}, נקבע לך תור חדש עבור ${serviceName} בתאריך ${format(dataAfter.start.toDate(), 'eeee, d MMMM', { locale: he })} בשעה ${format(dataAfter.start.toDate(), 'HH:mm')}.`;
            const dedupeKey = `${appointmentId}_admin_created`;
            const variables = {
            ...
            await sendPushNotification(clientId, 'clients', title, body, { route: '/my-appointments', notificationId: `new-${appointmentId}` });
            await enqueueWhatsAppMessage(dedupeKey, 'appointmentBookedByAdmin', variables, { clientId, title, body, data: { route: '/my-appointments', appointmentId } });
        }

        // 1. Appointment Approved
        else if (dataBefore?.status === 'pending' && dataAfter?.status === 'scheduled') {
            ...
            await sendPushNotification(clientId, 'clients', title, body, { route: '/my-appointments', notificationId: `approved-${appointmentId}` });
        ...
        // 2. Appointment Rescheduled by Admin
            ...
            await sendPushNotification(clientId, 'clients', title, body, { route: '/my-appointments', notificationId: `rescheduled-${appointmentId}` });
        ...
        // 3. Appointment Cancelled/Rejected by Admin
            ...
            await sendPushNotification(clientId, 'clients', title, body, { route: '/my-appointments', notificationId: `cancelled-${appointmentId}` });
