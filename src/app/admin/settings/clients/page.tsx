...
const TreatmentFormBuilder = ({ onSave, onCancel, initialData, services, categories, formType }: { onSave: (form: TreatmentFormTemplate) => void, onCancel: () => void, initialData?: TreatmentFormTemplate, services: Service[], categories: Category[], formType: 'treatment' | 'summary' }) => {
        ...
        <Card>
            ...
            <CardContent className="space-y-4">
                <div className="space-y-2">
                 ...
                 <div className="space-y-2">
                    <Label>שיוך לקטגוריות (אופציונלי)</Label>
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                        <Button variant="outline" className="w-full justify-between">
                            <span>
                                {(form.categoryIds?.length || 0) > 0
                                ? `${form.categoryIds?.length} קטגוריות נבחרו`
                                : 'בחר קטגוריות לשיוך...'}
                            </span>
                        ...
                        <DropdownMenuLabel>שייך קטגוריות</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        {categories.map(category => (
                            ...
                            onCheckedChange={(checked) => {
                                ...
                                if (checked) {
                                    setForm(prev => ({...prev, categoryIds: [...currentIds, category.id]}));
                                } else {
                                    setForm(prev => ({...prev, categoryIds: currentIds.filter(id => id !== category.id)}));
                                }
                            }}
                            >
                            {category.name}
                    ...
                    <p className="text-xs text-muted-foreground">
                        שיוך טופס לקטגוריה יאפשר הצעה אוטומטית לכל טיפול שנמצא תחת אותה קטגוריה.
                    </p>
                </div>
                <Separator />
                                     ...
                                     <div className="flex items-center gap-4 flex-wrap">
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                ...
                                                <SelectItem value="appointmentTreatmentsTable">טבלת טיפולים מהתור</SelectItem>
<SelectItem value="dynamicGrid">טבלת טיפולים דינמית</SelectItem>
                                                <Separator />
                                    ...
                                    {(field.type === 'select' || field.type === 'dynamicGrid') && (
                                         <div className="pl-4 space-y-2">
                                             <Label className="text-xs">{field.type === 'dynamicGrid' ? 'עמודות הטבלה' : 'אפשרויות'}</Label>
                                    ...
                                    {field.type === 'dynamicGrid' && (
                                        <div className="pl-4 space-y-2 border rounded-md p-3 bg-accent/20">
                                            <Label className="text-xs">מצב תצוגה</Label>
                                            <Select
                                                value={field.gridMode || 'matrix'}
                                                onValueChange={() => updateField(field.id, { gridMode: 'matrix' })}
                                            >
                                                <SelectTrigger className="w-[220px]">
                                                    <SelectValue />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    <SelectItem value="matrix">מצב תצוגה: Matrix</SelectItem>
                                                </SelectContent>
                                            </Select>
                                            <p className="text-xs text-muted-foreground">
                                                השורות יגיעו אוטומטית מהטיפולים שנבחרו בתור, והאפשרויות שהוגדרו יהפכו לעמודות.
                                            </p>
                                        </div>
                                    )}
                                    {field.type === 'image' && (
                                        ...
                                        <span className="font-semibold">{template.name}</span>
                                         {template.categoryIds && template.categoryIds.length > 0 && (
                                            <p className="text-xs text-primary max-w-xs truncate">
                                                {template.categoryIds.map(id => categories.find(c => c.id === id)?.name).filter(Boolean).join(', ')}
                                            </p>
                                        )}
                                    </div>
