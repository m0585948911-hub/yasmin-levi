      ...
      <CardContent className="space-y-6">
        {template.fields
          ...
          .map(field => {
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <ScrollArea className="h-24 w-full rounded-md border p-2">
                        <pre className="text-sm whitespace-pre-wrap font-sans">{field.label}</pre>
                      </ScrollArea>
                    </CardContent>
                  </Card>
            ...
            // ✅ FIX: make select options safe (no empty/undefined)
            const safeSelectOptions =
              field.type === "select"
                ...
                {field.type === 'signature' && (
                  <div className="space-y-2">
                    ...
                    {isRequired && !formData[field.id] && (
                      <p className="text-sm font-medium text-destructive">חתימה היא שדה חובה.</p>
                    )}
                  </div>
                )}
              </div>
...
const ViewCompletedFormDialog = ({
  isOpen, onOpenChange, instance, template
}: {
  isOpen: boolean,
                    ...
                    <div key={form.instanceId} className="flex items-center justify-between p-3 border rounded-md">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          הושלם בתאריך: {form.filledAt ? new Date(form.filledAt).toLocaleDateString('he-IL') : 'לא צוין'}
                        </p>
                      </div>
                      <Button variant="outline" size="sm" onClick={() => setViewingInstance(form)}>צפייה</Button>
                    </div>
                  ))}
                </div>
              ) : (
